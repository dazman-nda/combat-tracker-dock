...
export class CombatantPortrait {
    ...
    get img() {
        const customImage = this.actor?.getFlag?.(MODULE_ID, "carouselImage")
            ?? foundry.utils.getProperty(this.actor?.flags, `${MODULE_ID}.carouselImage`);
        ...
        const preferredImage = useActor ? this.actor?.img : this.combatant.img;
        const fallbackImage = useActor ? this.combatant.img : this.actor?.img;
        return [preferredImage, fallbackImage].find((image) => {
            return typeof image === "string" ? image.trim().length > 0 : image !== null && image !== undefined;
        }) ?? "";
    }

    get hasDefeatedStatus() {
        if (this.combatant?.isDefeated) return true;
        const defeatedStatus = CONFIG.specialStatusEffects?.DEFEATED;
        return Boolean(this.actor?.temporaryEffects?.some((effect) => {
            const statuses = effect.statuses;
            return Boolean(statuses && ((defeatedStatus && statuses.has(defeatedStatus)) || statuses.has("dead")));
        }));
    }
    ...
    getPortraitHealthResource() {
        if (!this.hasPermission) return null;

        const configuredResource = game.settings.get(MODULE_ID, "portraitResource");
        if (configuredResource) {
            const resource = this.getResource(configuredResource);
            if (this.isValidPortraitHealthResource(resource)) return resource;
        }

        // GUM stores HP at attributes.hp.value/max. The other paths keep the
        // health overlay useful when the dock is used with a different system.
        const candidates = [
            "attributes.hp.value",
            "hp.value",
            "resources.hp.value",
        ...
        for (const candidate of candidates) {
            const resource = this.getResource(candidate);
            if (this.isValidPortraitHealthResource(resource)) {
                return resource;
            }
        }
        return null;
    ...
    isValidPortraitHealthResource(resource) {
        if (!resource || resource.max === null || resource.max === undefined || resource.value === null || resource.value === undefined) return false;
        const max = Number(resource.max);
        const value = Number(resource.value);
        return Number.isFinite(max) && max > 0 && Number.isFinite(value);
    }

    getPortraitHealth(resource) {
        ...
        const positiveRatio = Math.min(Math.max(value / max, 0), 1);
        const multiplier = Number(game.settings.get(MODULE_ID, "portraitNegativeHealthMultiplier")) || 5;
        const negativeRatio = value <= 0
            ? Math.max(0, 1 - Math.min(Math.abs(value) / (max * multiplier), 1))
            : 0;

        return {
            isDead: this.hasDefeatedStatus,
            showPositive: value > 0,
            showNegative: value <= 0,
            positivePercentage: positiveRatio * 100,
            negativePercentage: negativeRatio * 100,
            color: this.getPortraitHealthColor(positiveRatio),
        };
    }

    getPortraitHealthColor(ratio) {
        const stops = [
            { at: 0, color: [230, 33, 33] },
            { at: 1 / 3, color: [255, 140, 0] },
            { at: 2 / 3, color: [255, 205, 0] },
            { at: 1, color: [65, 170, 125] },
        ];
    ...
    async getData() {
        ...
        if (hideDefeated && this.hasDefeatedStatus) return null;
        ...
        const portraitResource = this.getPortraitHealthResource();
        ...
        const turn = {
            ...
            defeated: this.hasDefeatedStatus,
            ...
            portraitHealth: game.settings.get(MODULE_ID, "portraitHealthBar")
                ? this.getPortraitHealth(portraitResource)
                : null,
